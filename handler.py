...
def generate_certificate(request, context):
    ...
    if not result["passed"]:
        ...
        if event.get('test_name_short') == "neo4j-3.x-certification_test": print("Assigning swag code")
...
def send_email(event, context):
    ...
    if event.get('test_name_short') == "neo4j-3.x-certification_test": email_title = 'Congratulations! You are now a Neo4j Certified Professional'
    template_name = 'email'

    if event.get('test_name_short') == "neo4j-4.x-certification_test": email_title = 'Congratulations! You are now Neo4j 4.0 Certified'
